...
$result = mysqli_query($conn, "select * from SL_menu where U_type='index'");
...
$sql="select * from SL_menu where U_type like 'text' and U_typeid=".intval($S_id)." limit 1";
...
$sql="select * from SL_text where T_id=".intval($S_id);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
$T_id=$row["T_id"];
...
$sql="select * from SL_psort ".$S_info." and S_show=1 limit 1";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
$S_idx=$row["S_id"];
...
$sql="select * from SL_menu where U_type='product' and U_typeid=".intval($S_idx);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["U_sub"]==0){
...
function CreateProductInfo($html, $P_id) {  //生成产品内容页
    ...
    $sql = "select * from SL_product,SL_psort where P_sort=S_id and P_id=" . intval($P_id) . " and S_show=1 order by P_order desc";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    if (mysqli_num_rows($result) > 0) {
        $P_idx = $row["P_id"];
    ...
    if ($S_sub != 0) {
        $sql = "select * from SL_psort where S_id=" . intval($S_sub);
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        if (mysqli_num_rows($result) > 0) {
    ...
    $sql = "select * from SL_menu where U_type='product' and U_typeid=" . intval($S_id);
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    if (mysqli_num_rows($result) > 0) {
        if ($row["U_sub"] == 0) {
    ...
    $sql = "select * from SL_product where P_sort=" . intval($S_id) . " order by P_order,P_id desc";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
    ...
    if ($html != "" && !is_null($html)) {
        ...
        $HTMLCode = str_Replace("{@SL_产品简述}", $P_short, $HTMLCode);
            ...
            if ($P_unlogin == 1) {
                $P_sc = $P_sc . "<input type='button' value='" . lang("立即购买/l/Buy Now") . "' onClick='buy(\$(\"#buy\").serialize(),\"".$C_dir."\")' class='buy'></form><div id=\"7pay\"><div id=\"goodcover\"></div><div id=\"code\"><script src=\"//7-pay.cn/" . $C_7PID . ".js?type=div&title=" . $P_title . "&content=分类：" . $S_title . "&contact=" . $P_contact . "&name=" . $P_name . "&address=" . $P_address . "&mobile=" . $P_mobile . "&remark=" . $P_remark . "&postcode=" . $P_postcode . "&qq=" . $P_qq . "&email=" . $P_email . "&input=false&list=0\"></script></div></div>";
            } else {
                $P_sc = $P_sc . "<input type='submit' value='" . lang("立即购买/l/Buy Now") . "' onClick='return check()' class='buy'> <input type='button' value='" . lang("加入购物车/l/Add Cart") . "' onClick='addcart(\$(\"#buy\").serialize(),\"".$C_dir."\")' class='cart'></form>";
            }
...
$xx=URLDecode($xx);
if($xx=="" || $xx=="0" || strpos($xx,"tag:")!==false || strpos($xx,"author:")!==false || strpos($xx,"date:")!==false || strpos($xx,"type:")!==false){
$S_info=" where S_id>0";
}else{
$S_info=" where S_id=".intval($xx);
}
$sql="select * from SL_nsort ".$S_info." and S_show=1";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
$S_idx=$row["S_id"];
...
$sql="select * from SL_menu where U_type='news' and U_typeid=".intval($S_idx);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["U_sub"]==0){
...
$sql="select * from SL_news,SL_nsort where N_sort=S_id and N_id=".intval($N_id)." and S_show=1 and N_sh=0 order by N_id desc";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
$N_idx=$row["N_id"];
...
$file=explode("|",$N_file);
$team=explode("|",$N_team);
$sql="select * from SL_menu where U_type='news' and U_typeid=".intval($S_id);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["U_sub"]==0){
...
$sql="select * from SL_news where N_sort=".intval($S_id)." and N_sh=0 order by N_order,N_id desc";
$result = mysqli_query($conn, $sql);
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
...
$N_Ntitle=lang(getrs("select * from SL_news where N_sh=0 and N_id=".intval($N_Nid),"N_title"));
}
if($N_Pid=="0"){
$N_Ptitle=lang("没有了/l/None");
}else{
$N_Ptitle=lang(getrs("select * from SL_news where N_sh=0 and N_id=".intval($N_Pid),"N_title"));
}
if($html!="" && !is_null($html)){
$HTMLCode=str_Replace("{@SL_新闻分类标题}",$S_title,$html);
$HTMLCode=str_Replace("{@SL_新闻分类英文标题}",$S_entitle,$HTMLCode);
...
$job_info="<div class=\"wrapx\"><table border=1 bordercolor=\"#EEEEEE\" cellspacing=0 frame=\"hsides\" rules=\"rows\" cellpadding=0 class=\"news_tab\"><tr><td><b>".lang("招聘职位/l/position")."：</b>".$job[0]."</td><td><b>".lang("招聘人数/l/Number")."：</b>".$job[1]."</td><td><b>".lang("工作地点/l/Place")."：</b>".$job[2]."</td></tr><tr><td><b>".lang("薪资水平/l/salary")."：</b>".$job[3]."</td><td><b>".lang("学历要求/l/Education")."：</b>".$job[4]."</td><td><b>".lang("经验要求/l/Experience")."：</b>".$job[5]."</td></tr><tr><td><b>".lang("年龄要求/l/Age")."：</b>".$job[6]."</td><td><b>".lang("性别要求/l/Gender")."：</b>".$job[7]."</td><td><b>".lang("语言要求/l/Language")."：</b>".$job[8]."</td></tr></table></div>";
...
$sql="select * from SL_form where F_id=".intval($S_id);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
...
$sql="select * from SL_menu where U_type='form' and U_typeid=".intval($S_id);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["U_sub"]==0){
...
$sql="select * from SL_menu where U_type='contact'";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["U_sub"]==0){
...
function CreateGuestbook($html){  //生成留言页面
    global $conn,$C_dirx;
$sql="select * from SL_menu where U_type='guestbook'";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["U_sub"]==0){
...
$i=0;
$sql="select * from SL_nsort where S_id>0 ".$S_info." and S_show=1 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);
...
if($S_id==0){
$sql2="select * from SL_nsort where S_sub=".$row["S_id"]." and S_show=1 order by S_order,S_id desc";
...
$i=0;
$sql="select * from SL_psort where S_id>0 ".$S_info." and S_show=1 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);
...
if($S_id==0){
$sql2="select * from SL_psort where S_sub=".$row["S_id"]." and S_show=1 order by S_order,S_id desc";
$result2 = mysqli_query($conn, $sql2);
...
global $conn,$C_dir,$C_dirx,$C_index,$C_html;
$result = mysqli_query($conn,"select * from SL_slide order by S_order,S_id desc");
if (mysqli_num_rows($result) > 0) {
$i=1;
while($row = mysqli_fetch_assoc($result)) {
        if(is_file($C_dirx.$row["S_pic"])){
...
if($U_sub!=0){
$sql="select * from SL_menu where U_sub=".$U_sub." and not U_sub=0 and U_hide=0 order by U_order";
$result = mysqli_query($conn, $sql);
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
...
$sql="select * from SL_menu where U_sub=".intval($U_id)." and not U_sub=0 and U_hide=0 order by U_order";
$result = mysqli_query($conn, $sql);
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
$style2=str_Replace("%左侧标题%",lang($row["U_title"]),$style);
...
if($xx=="0" || $xx==""){
$sql="select count(*) as count_num from SL_news,SL_nsort where N_sh=0 and N_sort=S_id and S_show=1";
}else{
$sql2="select  * from SL_nsort where S_show=1 and S_id=".intval($xx);
$result = mysqli_query($conn, $sql2);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["S_sub"]==0){
$sql="select count(*) as count_num from SL_news,SL_nsort where N_sh=0 and N_sort=S_id and S_show=1 and S_sub=".intval($xx);
}else{
$sql="select count(*) as count_num from SL_news,SL_nsort where N_sh=0 and N_sort=S_id and S_show=1 and N_sort=".intval($xx);
}
}
}
}else{
...
case "tag":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_tag like '%,".splitx($xx,":",1).",%'";
break;
case "author":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_author like '".splitx($xx,":",1)."'";
break;
case "date":

$sql="select count(*) as count_num from SL_news where N_sh=0 and year(N_date)=".date("Y",strtotime(splitx($xx,":",1)))." and month(N_date)=".date("m",strtotime(splitx($xx,":",1)))." and day(N_date)=".date("d",strtotime(splitx($xx,":",1)));
break;
case "type":
switch(splitx($xx,":",1)){

case "news":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_type=0";
break;
case "job":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_type=1";
break;
case "download":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_type=2";
break;
case "video":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_type=3";
break;
case "notice":
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_type=4";
default:
$sql="select count(*) as count_num from SL_news where N_sh=0 and N_type=0";
}
}
...
if($xx=="0" || $xx==""){
$sql="select count(*) as count_num from SL_product,SL_psort where S_type=".intval($S_type)." and S_show=1 and P_sort=S_id";
}else{
$sql2="select  * from SL_psort where S_type=".intval($S_type)." and S_id=".intval($xx);
$result = mysqli_query($conn, $sql2);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
if($row["S_sub"]==0){
$sql="select count(*) as count_num from SL_product,SL_psort where S_type=".intval($S_type)." and P_sort=S_id and S_show=1 and S_sub=".intval($xx);
}else{
$sql="select count(*) as count_num from SL_product,SL_psort where P_sort=S_id and S_show=1 and P_sort=".intval($xx);
}
}
}
$list="product_list";
...
$result = mysqli_query($conn, "select * from SL_menu where U_sub=0 and U_hide=0 order by U_order");
$j = 1;
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $result1 = mysqli_query($conn, "select * from SL_menu where U_sub=" . $row["U_id"] . " and U_hide=0 order by U_order");
        $i = 1;
        ...
        if ($row["U_type"] == "link" || ($row["U_url"] != "" && $row["U_url"]!="|" && !isnull($row["U_url"]))) {
            if(strpos($row["U_url"],"|")===false){
                $url=$row["U_url"];
                $target="blank";
            }else{
...
function product_sort_list($style, $S_sub, $num) {    //获取产品分类列表
    ...
    if ($S_sub != "0") {
        $result = mysqli_query($conn, "select * from SL_psort where " . $sub_info . " and S_type=" . intval($S_type) . " and S_show=1 order by S_order,S_id desc" . $num_info);
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
        ...
        $result = mysqli_query($conn, "select * from SL_psort where S_sub=0 and S_show=1 order by S_order,S_id desc");
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                $result2 = mysqli_query($conn, "select  * from SL_psort where S_sub=" . $row["S_id"] . " and S_type=" . intval($S_type) . " and S_show=1 order by S_order,S_id desc" . $num_info);
                if (mysqli_num_rows($result2) > 0) {
                    while ($row2 = mysqli_fetch_assoc($result2)) {
...
global $conn,$C_dir,$C_dirx,$C_html,$C_index;
$x=0;
$sql="select * from SL_text order by T_order,T_id desc";
$result = mysqli_query($conn, $sql);
...
$sql="select * from SL_nsort where ".$sort." and S_show=1 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);
...
if($S_id=="x"){
$sql1="select * from SL_news,SL_nsort where N_sh=0 and N_sort=S_id and S_show=1 and S_sub=".$row["S_id"]." order by N_order,N_id desc ".$num_info." ";
}else{
$sql1="select * from SL_news,SL_nsort where N_sh=0 and N_sort=S_id and S_show=1 and N_sort=".$row["S_id"]." order by N_order,N_id desc ".$num_info." ";
}
...
$sql="select * from SL_psort where ".$sort." and S_show=1 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
if($S_id=="x"){
$sql1="select * from SL_product,SL_psort where P_sort=S_id and S_sub=".$row["S_id"]." and S_show=1 order by P_order,P_id desc ".$num_info."";
}else{
$sql1="select * from SL_product,SL_psort where P_sort=S_id and P_sort=".$row["S_id"]." and S_show=1 order by P_order,P_id desc ".$num_info."";
}
$result1 = mysqli_query($conn, $sql1);
if (mysqli_num_rows($result1) > 0) {
while($row1 = mysqli_fetch_assoc($result1)) {
...
if(!is_numeric($S_id)){
$sql="select * from SL_psort where S_sub=0 and S_show=1 and S_type=0 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);

if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
$S_sublist=$S_sublist.$row["S_id"].",";
$sql2="select * from SL_psort where S_sub=".$row["S_id"]." and S_show=1 and S_type=0 order by S_order,S_id desc";
$result2 = mysqli_query($conn, $sql2);
...
$sql="select * from SL_psort where S_type=".intval($S_type)." and S_show=1 order by S_id asc limit 1";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
$S_aid=$row["S_id"];
...
$sql="select * from SL_psort where S_type=".intval($S_type)." and S_show=1 and S_id=".intval($S_id);
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
if(mysqli_num_rows($result) > 0) {
$S_sub=$row["S_sub"];
...
$x=0;
if($S_page==1){
$sql="select * from SL_product,SL_psort where P_sort=S_id and S_type=".intval($S_type)." ".$S_id_info." ".$show_info." and S_show=1 order by P_top desc,P_order asc,P_id desc ".$num_info." ";
}else{
$sql="select * from SL_product,SL_psort where P_sort=S_id and S_type=".intval($S_type)." ".$S_id_info." ".$show_info." and S_show=1 order by P_top desc,P_order asc,P_id desc limit ".$num*($S_page-1).",".$num;;
}
$result = mysqli_query($conn, $sql);
...
$x=0;
$sql="select * from SL_nsort where not S_sub=0 and S_show=1 order by S_id desc";
$result = mysqli_query($conn, $sql);
...
global $conn,$C_dir,$C_dirx,$C_html,$C_index;
$sql="select * from SL_psort where S_sub=0 and S_type=0 and S_show=1 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);

if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
$sql1="select * from SL_psort where S_sub=".$row["S_id"]." and S_type=0 and S_show=1 order by S_order,S_id desc";
$result1 = mysqli_query($conn, $sql1);
...
global $conn,$C_dir,$C_dirx,$C_html,$C_index;
$sql="select * from SL_nsort where S_sub=0 and S_show=1 order by S_order,S_id desc";
$result = mysqli_query($conn, $sql);

if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
$sql1="select * from SL_nsort where S_sub=".$row["S_id"]." and S_show=1 order by S_order,S_id desc";
$result1 = mysqli_query($conn, $sql1);
...
global $conn,$C_dir,$C_dirx,$C_index,$C_html;
if(is_Numeric($T_id)){
$sql="select * from SL_text where T_id=".intval($T_id);
}else{
switch($T_id){
...
if($top==0){
$sql="select * from SL_text order by T_order,T_id desc limit 1";
}else{
$sql="select * from (select * from SL_text order by T_order,T_id desc limit ".($top+1).")a order by T_order desc,T_id asc limit 1";
}
}
$result = mysqli_query($conn, $sql);
if(mysqli_num_rows($result) > 0) {
...
if($from1!="free"){
$infox=" and L_url not like 'http://www.s-cms.cn'";
}else{
$infox="";
}
if(is_numeric($S_id)){
...
$sql="select * from SL_link where L_id>0 ".$aa.$infox." order by L_id desc";
$result = mysqli_query($conn, $sql);
...
if(mysqli_num_rows($result) > 0) {
while($row = mysqli_fetch_assoc($result)) {
$sql1="select * from SL_link,SL_lsort where L_sort=S_id and S_id=".$row["S_id"]." order by L_id desc";
$result1 = mysqli_query($conn, $sql1);
...
global $conn,$C_html,$C_index,$C_dir;
$x=0;
$sql="select * from SL_form order by F_id desc";
$result = mysqli_query($conn, $sql);
...
function news_list2($title_tyle, $style, $num, $S_id, $order) {    //获取新闻列表
    ...
    if (strpos($S_id, "tag") === false && strpos($S_id, "date") === false && strpos($S_id, "author") === false && strpos($S_id, "type") === false) {
        ...
        $sql = "select S_id from SL_nsort where S_show=1 order by S_id asc limit 1";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($result);
        if (mysqli_num_rows($result) > 0) {
            $S_aid = $row["S_id"];
        ...
        if ($S_id == "0") {
            ...
            $sql = "select * from SL_nsort where S_show=1 and S_id=" . intval($S_id);
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_assoc($result);
            if (mysqli_num_rows($result) > 0) {
                $S_sub = $row["S_sub"];
    ...
    $x = 0;
    if ($S_page == 1) {
        $sql = "select * from SL_news,SL_nsort where N_sort=S_id " . $S_id_info . " " . $show_info . " and N_sh=0 and S_show=1 order by " . $orderby . " " . $num_info;
    } else {
        $sql = "select * from SL_news,SL_nsort where N_sort=S_id " . $S_id_info . " " . $show_info . " and N_sh=0 and S_show=1 order by " . $orderby . " limit " . $num * ($S_page - 1) . "," . $num;
    }
